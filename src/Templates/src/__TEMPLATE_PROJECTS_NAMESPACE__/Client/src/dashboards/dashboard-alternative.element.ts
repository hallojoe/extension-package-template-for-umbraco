...
export class ExampleDashboardAlternativeElement extends UmbElementMixin(LitElement) {
  @state()
  ...
  #getWorkspaceContext() {
    if (!this.#workspaceContext) {
      throw new Error("Workspace context is not available. Check that the context manifest is registered in the bundle.");
    }
    ...
    this.consumeContext(
      ...
      (workspaceContext) => {
        ...
        this.observe(
          ...
          (name) => {
            this._yourName = name ?? "Press the button!";
          },
          "_workspaceName",
        );

        this.observe(
          workspaceContext?.time,
          (time) => {
            this._timeFromMrWolf = time ? new Date(time) : undefined;
          },
          "_workspaceTime",
        );
  ...
  #onClickWhoAmI = async (ev: Event) => {
    ...
    const { error } = await this.#getWorkspaceContext().loadUser();
  ...
  #onClickWhatsTheTimeMrWolf = async (ev: Event) => {
    ...
    const { error } = await this.#getWorkspaceContext().loadTime();
  ...
  #onClickWhatsMyName = async (ev: Event) => {
    ...
    const { error } = await this.#getWorkspaceContext().loadName();
...
export default ExampleDashboardAlternativeElement;

declare global {
  interface HTMLElementTagNameMap {
    "example-dashboard-alternative": ExampleDashboardAlternativeElement;
  }
}
